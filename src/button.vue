<template>
  <button class="g-button" :class="{[`icon-${iconPosition}`]: true, disabled: loading}" @click="$emit('click')">
...
<style lang="scss" scoped>
  ...
  .g-button {
    box-sizing: border-box; transition: .3s cubic-bezier(.645, .045, .355, 1);
    ...
    &.circle { width: 28px; height: 28px; padding: 0; border-radius: 50%; }

    &.lg {
      &.circle { width: 32px; height: 32px; padding: 0; font-size: $font-size-ll; }
      padding: 4px 15px 5px 15px; font-size: $font-size-l; border-radius: $button-border-radius-l;
    }
    &.sm {
      &.circle { width: 22px; height: 22px; padding: 0; }
      padding: 1px 7px; border-radius: $button-border-radius-s;
    }

    &.primary {
      color: $primary-button-color; border-color: $primary-button-border-color; background: $primary-button-background-color;
      svg { fill: white; }
      &:hover {
        svg { fill: $primary-button-hover-color; }
        color: $primary-button-hover-color; border-color: $primary-button-hover-border-color; background: $primary-button-hover-background-color;
      }
      &:active {
        svg { fill: $primary-button-active-color; }
        color: $primary-button-active-color; border-color: $primary-button-active-border-color; background: $primary-button-active-background-color;
      }
    }

    &.ghost {
      svg { fill: $button-color; }
      color: $button-color; border-color: $button-border-color; background: white;
    }
    ...
    &:active {
      ...
      color: $button-active-color; border: 1px solid $button-active-border-color; background: $button-active-background-color;
    }
    &.disabled, &.primary.disabled {
      color: #ccc; background-color: #f3f5f7; border-color: #d9d9d9; cursor: not-allowed;
    }
  }
</style>
